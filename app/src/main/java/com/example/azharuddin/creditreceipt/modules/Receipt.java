...
public class Receipt {
    public void evaluate(LandingActivity activity, View itemView) {
        ViewPager viewPager = itemView.findViewById(R.id.receipt_viewpager);
    ...
    private void setupViewPager(LandingActivity activity, ViewPager viewPager) {
        ReceiptViewPagerAdapter adapter = new ReceiptViewPagerAdapter(activity.getSupportFragmentManager());
        adapter.addFragment(addFragment.newInstance("User"), "ADD");
        adapter.addFragment(viewFragment.newInstance("User"), "View");
        viewPager.setAdapter(adapter);
    }

    public class ReceiptViewPagerAdapter extends FragmentPagerAdapter {
        private final List<Fragment> mFragmentList = new ArrayList<>();
        private final List<String> mFragmentTitleList = new ArrayList<>();

        ReceiptViewPagerAdapter(FragmentManager manager) {
            super(manager);
        }
